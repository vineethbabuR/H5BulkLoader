 ...
 * The current extension handles csv file processing using vanilla javascript
 */
...
var BulkLoader = (function () {
   ...
   <div>
        ...
        $("#jsonFile").on('change', function (e) {
            ...
            reader.onload = function (evt) {
                ...
                //_this.payload = evt.target.result;
                 ...
                 /*Handle CSV processing here, replace object keys from csv headers to M3 API Field names*/
                let res = evt.target.result;
                console.log(_this.csvProcessor(res));
                _this.payload = _this.csvProcessor(res);
    ...
    BulkLoader.prototype.csvProcessor = function csvProcessor(records) {

            //read top level props from filename and sheetname
            let data = {"program": "MMS301MI",
            ...
            let result = [];
            let headers = lines[0].split(',').map(head => head.trim())

            for(let i = 1; i < lines.length; i++){
                let obj = {};
                let currentLine = lines[i].split(',').map(ln => ln.split('\r'))[0];

                for(let j = 0; j < headers.length; j++){
                    obj[headers[j]] = currentLine[j]
                }

                obj.WHLO = obj['Warehouse']
                obj.STNB = obj['Physical_inventory_number']
                obj.STRN = obj['Physical_inventory_line']
                obj.STQI = obj['Physical_inventory_quantity']
                obj.STBT = obj['Onhand_balance_to_compare']
                obj.STAG = obj['Status_physical_inventory']

                delete obj['Warehouse']
                delete obj['Physical_inventory_number']
                delete obj['Physical_inventory_line']
                delete obj['Physical_inventory_quantity']
                delete obj['Onhand_balance_to_compare']
                delete obj['Status_physical_inventory']

                //result.push(obj)
                data.transactions.push({
                    transaction:'UpdStockTake',
                    record:obj
                })
            }
